...
- 标准结构是在 [#基本结构](#基本结构) 基础上进一步完善的 ** `html` ** 结构，如下所示：
...
> [!WARNING] 备注
> 1. 这些不常用的文本标签，编写代码时不用过于纠结（其实不用也行，主要就是用 [#1-常用标签](#1-常用标签)）
> 2. `<blockquote>` 与 `<address>` 是**块级元素**，其它的文本标签，都是**行内元素**
> 3. 有些语义感不强的标签，我们很少使用，例如：`<small>`、`<b>`、`<u>`、`<q>`、`<blockquote>`
> 4. 由于 html 标签实在太多，我们只需记住 [#排版标签](#排版标签) 和 [#1-常用标签](#1-常用标签) 即可
...
1. 一个完整的表格由：**表格标题、表格头部、表格主体、表格脚注**四部分组成。
   ![HTML\_table\_structure](assets/1-html4/HTML_table_structure.png)
2. 表格涉及到的标签：
	- **`table`**：表格
	- **`caption`**：表格标题（没有属性）
	- **`thead`**：表格头部
...
2. `colspan`：指定要跨的列数。
3. 动手练习：
   ![table example](assets/1-html4/HTML_table_example.png)
...
![form structure](assets/1-html4/HTML_form_structure.png)
...
| `lang`  |                   给标签指定语言，具体规范可参考 [](HTML4#六、HTML%20设置语言|六、HTML设置语言)                   |

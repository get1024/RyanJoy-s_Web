...
Last edited at: 2024年05月01日 11:00:31
title: CSS2笔记
---
# 一-CSS 基础
## 1-简介
...
### 2.3-HTML 元素之间的关系
- 分为：[[CSS#①父元素|父元素]]、[[CSS#②子元素|子元素]]、[[CSS#③祖先元素|祖先元素]]、[[CSS#④后代元素|后代元素]]、[[CSS#⑤兄弟元素|兄弟元素]]
#### 2.3.1-父元素
...
>3. 儿子、孙子、重孙子……都算是后代；
>4. 结构一定要符合之前讲的 `HTML` 嵌套要求，例如：不能 `p` 中写 `h1~h6`。（详见 [HTML 排版标签](../HTML/HTML4.md#2-排版标签)）
### 2.5-子代选择器
...
- 常用的伪类选择器：[目标伪类](CSS.md#_5目标伪类-了解)、
#### 2.8.1-动态伪类
1. `:link` 超链接**未被访问**的状态；
